(function () {
    ...
    lI: function (a) {
      ...
      for (var g = 0; g < c; g++) {
        ...
        if (!i.ZH && !(i.mp && !this.map.Ub() || i.iM && this.map.Ub())) {
          ...
          w = [];
          for (d.LB = []; n < v; n++)
            for (y = l; y < e; y++) w.push([n, y]), d.LB.push({
              x: n,
              y: y
            });
  ...
  Vc.getTilesUrl = function (a, b, c) {
    ...
    // tdir = 'http://pub.winfo.cc:2001/winfoMapService?REQUEST=GetMap&SERVICE=CacheMap'
    // return tdir + "&Y=" + a + "&X=" + d + "&LEVEL=" + b + '&LAYERS=BaiduCS0615';
    tdir = 'http://198.18.11.130:3001/winfoMapService?REQUEST=GetMap&SERVICE=CacheMap'
    return tdir + "&Y=" + a + "&X=" + d + "&LEVEL=" + b + '&LAYERS=BaiduCS0615';
    /*
    	c = Uc[c];
    this.map.dE() && (e = 2);
